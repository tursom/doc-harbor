...
func migrate(ctx context.Context, db *sql.DB) error {
	stmts := []string{
		...
		`CREATE TABLE IF NOT EXISTS ai_secrets (
			...
			last4 TEXT NOT NULL,
			created_by_viewer TEXT NOT NULL DEFAULT '',
			updated_by_viewer TEXT NOT NULL DEFAULT '',
			created_at TEXT NOT NULL,
			updated_at TEXT NOT NULL
		)`,
		`CREATE TABLE IF NOT EXISTS ai_config_versions (
			...
			created_by_viewer TEXT NOT NULL DEFAULT '',
			published_by_viewer TEXT NOT NULL DEFAULT '',
			created_at TEXT NOT NULL,
			updated_at TEXT NOT NULL,
		...
		`CREATE INDEX IF NOT EXISTS idx_ai_config_versions_status ON ai_config_versions(status, version)`,
		...
		`CREATE INDEX IF NOT EXISTS idx_ai_sessions_updated ON ai_sessions(archived_at, updated_at)`,
		...
		`CREATE TABLE IF NOT EXISTS ai_agent_runs (
			...
			scope_json TEXT NOT NULL DEFAULT '{}',
			retrieval_plan_json TEXT NOT NULL DEFAULT '',
			service_candidate_count INTEGER NOT NULL DEFAULT 0,
			evidence_count INTEGER NOT NULL DEFAULT 0,
			code_evidence_count INTEGER NOT NULL DEFAULT 0,
			...
			model TEXT NOT NULL DEFAULT '',
			provider_name TEXT NOT NULL DEFAULT '',
			provider_failover_json TEXT NOT NULL DEFAULT '',
			model_route_json TEXT NOT NULL DEFAULT '',
			...
			error_message TEXT NOT NULL DEFAULT '',
			FOREIGN KEY(session_id) REFERENCES ai_sessions(id) ON DELETE CASCADE,
			FOREIGN KEY(user_message_id) REFERENCES ai_messages(id) ON DELETE CASCADE
		)`,
		`CREATE INDEX IF NOT EXISTS idx_ai_agent_runs_session ON ai_agent_runs(session_id, started_at)`,
		`CREATE INDEX IF NOT EXISTS idx_ai_agent_runs_status ON ai_agent_runs(status, current_state)`,
		`CREATE TABLE IF NOT EXISTS ai_agent_steps (
			...
			model TEXT NOT NULL DEFAULT '',
			provider_name TEXT NOT NULL DEFAULT '',
			model_route_reason TEXT NOT NULL DEFAULT '',
			escalated_from_step_id INTEGER NOT NULL DEFAULT 0,
		...
		`CREATE INDEX IF NOT EXISTS idx_ai_agent_steps_run ON ai_agent_steps(run_id, id)`,
		`CREATE TABLE IF NOT EXISTS ai_service_candidates (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			run_id INTEGER NOT NULL,
			message_id INTEGER NOT NULL DEFAULT 0,
			service_profile_id INTEGER NOT NULL DEFAULT 0,
			repo_id INTEGER NOT NULL,
		...
		`CREATE INDEX IF NOT EXISTS idx_ai_service_candidates_run ON ai_service_candidates(run_id, score)`,
		`CREATE TABLE IF NOT EXISTS ai_message_citations (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			message_id INTEGER NOT NULL,
			index_snapshot_id INTEGER NOT NULL DEFAULT 0,
			chunk_id INTEGER NOT NULL DEFAULT 0,
			api_symbol_id INTEGER NOT NULL DEFAULT 0,
			repo_id INTEGER NOT NULL,
		...
		`CREATE INDEX IF NOT EXISTS idx_ai_message_citations_message ON ai_message_citations(message_id, id)`,
	}
